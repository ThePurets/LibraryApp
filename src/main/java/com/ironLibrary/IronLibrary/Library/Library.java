 ...
 */
    ...
    public void issueBookToStudent() throws IOException {
        ...
        do {
            ...
            if (bookId == true) {
                ...
                //debería volver al menú principal o repreguntar?
            }
        } while (isbnTrue);
    ...
    public void listBooksByUsn() throws IOException {
        ...
        if(studentRepository.findById(usn).isPresent()){
            ...
            student1 = studentRepository.findById(usn).get();
        }

        if (issueRepository.findByIssueStudent(student1).isPresent()) {

            issue1 = issueRepository.findByIssueStudent(student1).get();

            System.out.println("Book Title        Student Name       Return Date");
            System.out.println(issue1.getIssueBook().getTitle() + "    " + issue1.getIssueStudent().getName() + "    " + issue1.getReturnDate());

        } else throw new IllegalArgumentException("No usn valid.");
    }
}
